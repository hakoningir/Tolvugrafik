     ...
     window.addEventListener("keydown", function(e){
         switch( e.code ) {
            ...
            case "ArrowDown":	// niður ör
                zDist -= 1.0;
                break;
            case "KeyZ":	// z - snýr stöpli áfram
			    theta[0] = Math.min(180, theta[0]+5);
                break;
            case "KeyX":	// x - snýr stöpli afturábak
			    theta[0] = Math.max(-180, theta[0]-5);
                break;
            case "KeyA":	// a - snýr neðri armi
			    theta[1] = Math.min(80, theta[1]+5);
                break;
            case "KeyS":	// s - snýr neðri armi
			    theta[1] = Math.max(-80, theta[1]-5);
                break;
            case "KeyQ":	// q - snýr efri armi
			    theta[2] = Math.min(135, theta[2]+5);
                break;
            case "KeyW":	// w - snýr efri armi
			    theta[2] = Math.max(-135, theta[2]-5);
                break;
            case "ArrowLeft":   // vinstri ör
                theta[3] = Math.min(135, theta[3]+5);
                break;
            case "ArrowRight":   // hægri ör
                theta[3] = Math.max(-135, theta[3]-5);
                break;
         }
     }  );  
...
var render = function() {
    ...
    modelViewMatrix  = mult(modelViewMatrix, rotateZ( theta[MidArm] ) );
    midArm();
